...
def evaluation(args, sample):
    ...
    with torch.no_grad():
        for eval_id in range(args.eval_num):
            eval_env.reset(seed=args.seed + eval_id)
            ob = eval_env.reset()
            done = False
            gamma = 1.0
            while not done:
                ...
                _, action, _, _ = policy.act(ob, None, None, deterministic=True)  
                ob, _, done, _, info = eval_env.step(action)
                objs += gamma * info['obj']
                if not args.raw:
                    gamma *= args.gamma
    eval_env.close()
...
def evaluate_parallel(actor_critic, ob_rms, env_name, seed, num_processes, eval_log_dir,
             device, nr_eval_episodes=10):
    ...
    while len(eval_episode_rewards) < nr_eval_episodes:
        with torch.no_grad():
    ...
    objs = np.array([np.mean(eval_episode_efficiency_scores),
                     np.mean(eval_episode_fairness_scores)])
    return objs
...
'''
def MOPG_worker(args, task_id, task, device, iteration, num_updates, start_time, results_queue=None, done_event=None):
    ...
    results = {}
    results['offspring_batch'] = []
    
    print(f"Start iterations at task {task_id} with weights at time {time.time()}")
    for j in range(start_iter, final_iter):
        ...
        for step in range(args.num_steps):
            ...
            for idx, info in enumerate(infos):
                ...
                episode_obj[idx] = info['obj'] if episode_obj[idx] is None else episode_obj[idx] + info['obj']
                if 'episode' in info.keys():
        ...
        try:
            ...
            env_params['ob_rms'] = None
        try:
            env_params['ret_rms'] = deepcopy(envs.ret_rms) if envs.ret_rms is not None else None
        except AttributeError:
            env_params['ret_rms'] = None
        try:
            env_params['obj_rms'] = deepcopy(envs.obj_rms) if envs.obj_rms is not None else None
        except AttributeError:
            env_params['obj_rms'] = None
        # evaluate new sample
        sample = Sample(env_params, deepcopy(actor_critic), deepcopy(agent))
        print(f"Evaluate at task {task_id} with weights at time {time.time()}")
        if (j + 1) % 4 == 0:
            objs = evaluate_parallel(actor_critic=sample.actor_critic, ob_rms=None,
                                    env_name=args.env_name, seed=args.seed, num_processes=10,
                                    eval_log_dir=None, device=device, nr_eval_episodes=args.eval_num)
        else:
            objs = None
        sample.objs = objs
        offspring_batch.append(sample)

        if True or (args.rl_log_interval > 0 and (j + 1) % args.rl_log_interval == 0 and len(episode_rewards) > 1):
            if task_id == 0:
        ...
        if (j + 1) % args.update_iter == 0 or j == final_iter - 1:
            ...
            results['offspring_batch'].append(offspring_batch)
            offspring_batch = []
    return results