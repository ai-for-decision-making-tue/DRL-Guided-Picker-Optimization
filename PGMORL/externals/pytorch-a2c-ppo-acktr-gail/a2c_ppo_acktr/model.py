...
from .model_custom import invariantMLP_WITH_EMBEDDING_PER_CLASS_ACTOR,\
    InvariantMLP_PER_CLASS_PPO_CRITIC,\
        CREATE_SUPERVISED_OUTPUTinvariantMLP_WITH_EMBEDDING_PER_CLASS_ACTOR
...
class Policy_Create_DATA(Policy):
    def __init__(self, obs_shape, action_space, base=None, base_kwargs=None, obj_num=1):
        super(Policy, self).__init__()
        if base_kwargs is None:
            base_kwargs = {}
        base=GNNBase_CREATE_DATA
        if base is None:
            if len(obs_shape) == 3:
                base = CNNBase
            elif len(obs_shape) == 1:
...
class GNNBase(NNBase):
    def __init__(self, in_channels_fair, in_channels_efficient,
        ...
        super(GNNBase, self).__init__(recurrent=False, recurrent_input_size=1,hidden_size=1)
        self.actor = invariantMLP_WITH_EMBEDDING_PER_CLASS_ACTOR(
            in_channels_fair, in_channels_efficient, hidden_channels, emb_channels,
            hidden_after_emb, out_channels
        )
        self.critic = InvariantMLP_PER_CLASS_PPO_CRITIC(
            in_channels_fair, in_channels_efficient, hidden_channels, out_channels
        )
    
    def forward(self, x, state=None, info={}):
        return self.critic(x, state, info), self.actor(x, state, info), None
